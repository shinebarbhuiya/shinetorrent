...
def cancel_mirror(update, context):
    ...
    if dl.status() == "Uploading...📤":
        sendMessage("Upload in Progress, You Can't Cancel It.",
                    context.bot, update)
        return
    elif dl.status() == "Archiving...🔐":
        sendMessage("Archival in Progress, You Can't Cancel It.",
                    context.bot, update)
        return
    elif dl.status() == "Extracting...📂":
        sendMessage("Extract in Progress, You Can't Cancel It.",
                    context.bot, update)
        return
    else:
...
def cancel_all(update, context):
    ...
    while True:
        ...
        if dl:
            ...
            break
    delete_all_messages()
    sendMessage(f'{count} Download(s) has been Cancelled!',
                context.bot, update)
